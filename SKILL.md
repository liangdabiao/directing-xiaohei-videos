...
description: Use when turning copy, notes, articles, or topics into approximately one-minute Chinese-narrated "Xiao Hei" (小黑) motion videos — deadpan absurd white-canvas explainer clips with the solid-black character. Default production is the FREE pipeline (Agnes keyframe images + agnes-video keyframe mode + edge-tts, ¥0); paid apiz pipeline (minimax/h3) is fallback. Covers storyboard proposals, prompt packages, production pipeline, and QA.
---
...
Turn one source into a confirmed director's proposal (Phase A) and then six scenes of production prompts (Phase B) for roughly ten-second clips starring 小黑 — the solid-black deadpan creature — preserving the source's meaning while strengthening its hook, progression, and closing callback. 默认用免费管线产出（¥0）：每场=一张首帧图 + 一段 keyframe 动画 + edge-tts 旁白。

小黑视频 = 火柴人视频的骨架 + 小黑配图的灵魂：

- 骨架（来自 directing-stickman-videos，已验证）：Setup gate → Phase A 提案 → 批准门 → Phase B 六场提示词包 → 生成 → 合成。三拍节奏、场间交接、负向锁、旁白 audio-only 全部保留。
...
- theme 固定为白底黑小黑。黑色背景会让实心黑色小黑隐形，不提供暗色主题；如用户坚持暗色，解释后改用"反白小黑"（白色实心+黑点眼）并按全局变更重新提案
- 生成管线：默认免费方案（Agnes keyframe + edge-tts，¥0）；用户点名付费或免费不可用时才走 apiz h3
...
5. Only after approval of the current Phase A, read `references/video-prompt-contract.md` and produce Phase B（默认免费管线：每场首帧图提示词+运动提示词）。
6. When the user asks to actually produce the video, read `references/production-pipeline.md`（默认免费管线步骤；付费 h3 兜底参数也在其中）。
...
- 旁白 240–270 汉字，六场，每场 35–50 字 ≈ 8–12 秒（免费管线单场时长须落在 4–12 秒参数范围内）。
...
- 提示词自包含口径：首帧图提示词重复所有关键锁（白底、手绘线、小黑外形、禁文字）；运动提示词只写动作与变化（风格由首帧锁定），但必须带 no new objects / no text。付费管线的 12 项完整提示词则全部重复。
- 不虚构数据、引语、统计和产品主张。
